...
def networkx_to_katanagraph(x: NetworkXGraph, **props) -> KatanaGraph:
    ...
    aprops = NetworkXGraph.Type.compute_abstract_properties(
        x, {"node_dtype", "node_type", "edge_dtype", "edge_type", "edge_has_negative_weights", "is_directed",},
    )
...
def katanagraph_to_networkx(x: KatanaGraph, **props) -> NetworkXGraph:
    ...
    if isinstance(edge_weights[0], np.int64):
        ...
        elist = [(nid, pg.get_edge_dest(j), float(edge_weights[j])) for nid in pg for j in pg.edge_ids(nid)]
    elif isinstance(edge_weights[0], np.bool_):
        elist = [(nid, pg.get_edge_dest(j), bool(edge_weights[j])) for nid in pg for j in pg.edge_ids(nid)]
    ...
    if x.node_weight_prop_name:
        ...
        nx.set_node_attributes(graph, {node: wgt for node, wgt in zip(nodeid_map, nlist)}, name="weight")
